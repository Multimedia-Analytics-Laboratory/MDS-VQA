...
class RepeatRandomSampler(Sampler):
    """
    Distributed sampler that ensures each GPU's batch can come from different datasets.
    ...
    Args:
        ...
        per_gpu_batch_size (`int`, *optional*, defaults to `8`):
            Batch size per GPU.
        num_gpus (`int`, *optional*, defaults to `16`):
            Number of GPUs.
    ...
    def __init__(
        ...
        for idx, data in enumerate(data_source):
            ...
            image_name = data["image_path"][0].split("/")[-1]  # Assuming single image per data point
    ...
    def __iter__(self):
        # Create copies of indices that we can modify
        ...
        # Create lists of dataset names for random selection
        ...
        while True:
            ...
            for gpu_idx in range(self.num_gpus):
                ...
                # Shuffle the dataset names for random selection
                ...
                if selected_dataset in prefix_dataset_names:
                    ...
                    # Skip if not enough indices with this prefix for one GPU
                    ...
                    # Shuffle and take per_gpu_batch_size elements
                    ...
                    # Handle regular sampling for other datasets
                    indices = remaining_regular_indices[selected_dataset]
                    
                    # Skip if not enough indices for one GPU
                    if len(indices) < self.per_gpu_batch_size:
                    ...
                    # Remove dataset if exhausted
                    if not remaining_regular_indices[selected_dataset]:
                        regular_dataset_names.remove(selected_dataset)
                
                # Assign batch to this GPU
                gpu_batches[gpu_idx] = batch_indices
            
            # If we couldn't fill all GPU batches, we're done
            if not any(gpu_batches):
                break
            
            # Yield batches for all GPUs, with repeats
            for _ in range(self.repeat_count):
                for gpu_batch in gpu_batches:
                    if gpu_batch:  # Only yield if this GPU got a batch
...
class VLMGRPOTrainer(Trainer):
    ...
    def _get_train_sampler(self) -> Sampler:
        ...
        return RepeatRandomSampler(
            data_source=self.train_dataset,
            mini_repeat_count=self.num_generations,
            per_gpu_batch_size=self.args.per_device_train_batch_size // self.num_generations,
